...
- Release and package installation must not enable/start the service, pair a
  ...
  user state is preserved across package install, upgrade, and removal.

## Tasks

### Concise end-user README

- [ ] Rewrite `README.md` to open with a brief description of who the bridge is for and why it is useful, including local read-only forwarding through Apple's native ANCS protocol, no cloud/telemetry, and the security/privacy advantages.
- [ ] Add a short AUR-first quick start for an end user who only wants to install, confirm the iPhone during setup, enable the user service, and check that it is ready; keep teardown and the optional phone-audio behavior discoverable without exposing internal implementation detail.
- [ ] Add one short integration section covering the stable JSON/JSONL commands and systemd user service, then retain only essential platform support, troubleshooting, development, license, and experimental-adapter caveats; remove duplicated architecture, protocol, manual-install, and exhaustive acceptance detail from the landing page.
